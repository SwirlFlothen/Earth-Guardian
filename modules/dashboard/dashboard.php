...
// API Key for OpenWeatherMap (for both AQI and Weather)
$openweathermap_api_key = '4f041f825bf295a234efa9affb820581'; // Replace with your actual API key
...
$aqi_json_dhaka = file_get_contents($aqi_url_dhaka);
if ($aqi_json_dhaka) {
    $aqi_data['Dhaka'] = json_decode($aqi_json_dhaka, true);
}
...
$aqi_json_chattogram = file_get_contents($aqi_url_chattogram);
if ($aqi_json_chattogram) {
    $aqi_data['Chattogram'] = json_decode($aqi_json_chattogram, true);
}
...
$weather_json_dhaka = file_get_contents($weather_url_dhaka);
if ($weather_json_dhaka) {
    $weather_data['Dhaka'] = json_decode($weather_json_dhaka, true);
}
...
$weather_json_chattogram = file_get_contents($weather_url_chattogram);
if ($weather_json_chattogram) {
    $weather_data['Chattogram'] = json_decode($weather_json_chattogram, true);
}

// NASA EPIC API Integration
$nasa_api_key = '36sN6N5wSt9Y7EDJ5cZKGgeTAbmyd6IEUUkQYLU4';
$epic_data = [];
$epic_image_url = '';
$epic_error_message = '';
...
$ch = curl_init();
curl_setopt($ch, CURLOPT_URL, $epic_data_url);
curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
$epic_json = curl_exec($ch);

if (curl_errno($ch)) {
    $epic_error_message = "cURL Error: " . curl_error($ch);
} else {
    $http_code = curl_getinfo($ch, CURLINFO_HTTP_CODE);
    if ($http_code !== 200) {
        $epic_error_message = "HTTP Error: " . $http_code . " - " . $epic_json; // Display API error response
    } else {
...
$power_json = file_get_contents($power_api_url);

if ($power_json === false) {
    $power_error_message = "Failed to fetch data from NASA POWER API. Check internet connection or API endpoint.";
} else {
...
<head>
    ...
    <title>Dashboard - Earth Guardians</title>
...
<body>
    ...
    <nav class="navbar">
        ...
        <ul class="nav-links">
            <li><a href="dashboard.php" title="Dashboard" aria-label="Dashboard"><i class="fas fa-tachometer-alt"></i><span class="link-text">Dashboard</span></a></li>
    ...
    <div class="dashboard-container">
        ...
        <div class="nasa-epic-data card">
            ...
            <?php elseif (isset($epic_error_message)): ?>
    ...
    <script>
        ...
        const aqiLabels = Object.keys(aqiData);
    ...
    <script>
        ...
        const weatherLabels = Object.keys(weatherData);
